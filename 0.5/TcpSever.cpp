...
// namespace lidan{
 ...
 TcpSever::TcpSever(EventLoop * loop, int port)
 	:loop_(loop),
 	port_(port),
 	listenfd_(SocketBind(port_)),
 	listenchannel_(new Channel(loop_, listenfd_, EPOLLIN | EPOLLET))
 {
	 printf("\nlistenfd_:%d\n", listenfd_);
...
void TcpSever::Start(){
	...
	loop_->AddtoEpool(listenchannel_);
...
void TcpSever::NewConnect(){//监听事件被触发时，将会执行此函数
