...
public:
	...
	void AddtoEpool(SP_Channel newchannel);//将一个新的channel加入到epoll中
	void UptoEpool(SP_Channel oldchannel);//当epool中的事件相应过之后，需要重新将此事件添加到epoll中
	void RemovetoEpool(SP_Channel thechannel);//移除epoll中的channel
...
private:
	...
	std::vector<SP_Channel> ChannelList_;//epool中返回的需要处理的I/O事件的channel。都存储在这里
